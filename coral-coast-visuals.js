// CORAL_COAST_VISUALS_V16 — global legacy-base suppression + branded title/buttons
(()=>{
 ...
 function installTopBrandBanner(){
  ...
  img.src='assets/critter-clash-tower-defence-banner.png?v=16';
 ...
 function installPlayNowArtwork(){
  ...
  const img=document.createElement('img');
  img.src='assets/critter-clash-play-now-button.png?v=16';
  img.alt='Play Now';
  img.draggable=false;
 ...
 // Use the transparent jungle PNG as a true frame around the existing button.
 // Preserve the original text and click handler, but remove the old green pill so
 // the transparent parts of the artwork stay transparent and are not squashed.
 function installContinueBattleBorder(){
  const buttons=[...document.querySelectorAll('button')];
  const btn=buttons.find(b=>/continue\s*battle/i.test((b.textContent||'').trim()));
  if(!btn||btn.dataset.ccContinueBorder==='2')return;
  btn.dataset.ccContinueBorder='2';
  btn.style.cssText+='background-color:transparent!important;background-image:url("assets/critter-clash-continue-battle-border.png?v=16")!important;background-size:contain!important;background-position:center!important;background-repeat:no-repeat!important;border:0!important;box-shadow:none!important;height:92px!important;min-height:92px!important;padding:18px 70px!important;overflow:visible!important;display:flex!important;align-items:center!important;justify-content:center!important;';
 }
 ...
 const s=document.createElement('script');s.src='regions-8-10.js?v=16';s.onload=()=>{const u=document.createElement('script');u.src='regions-8-10-ui.js?v=16';document.body.appendChild(u)};document.body.appendChild(s);
})();
