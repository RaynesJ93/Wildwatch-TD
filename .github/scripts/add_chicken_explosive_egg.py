...
# Preserve the Chicken card's CURRENT balance values and only replace its description.
...
# Add combat behavior if it is not already present.
...
slow_line='if(e.chickenSlowTimer>0)e.chickenSlowTimer=Math.max(0,e.chickenSlowTimer-dt);'
if slow_line not in s:
    ...
    s=s.replace(oldslow,oldslow+slow_line,1)

mult='*(e.chickenSlowTimer>0?.80:1)'
if mult not in s:
    oldmult='*(e.zebraSlowTimer>0?.75:1)'
    if oldmult not in s:
        raise SystemExit('Enemy movement multiplier anchor not found')
    s=s.replace(oldmult,oldmult+mult,1)
...
# If an older partial implementation was already present, add a harmless marker comment.
if MARK not in s:
    marker_anchor='chicken:{name:"Chicken"'
    pos=s.find(marker_anchor)
    if pos<0:
        raise SystemExit('Chicken marker anchor not found')
