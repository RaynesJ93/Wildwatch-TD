from pathlib import Path
p=Path('index.html'); s=p.read_text()
# Slow Hedgehog attack: one five-needle stack every 3 seconds.
s=s.replace('hedgehog:{name:"Hedgehog",emoji:"🦔",rarity:"Common",cost:60,range:135,rate:1,dmg:12,color:"#76543b"','hedgehog:{name:"Hedgehog",emoji:"🦔",rarity:"Common",cost:60,range:135,rate:3,dmg:12,color:"#76543b"',1)
old='''  let target=null,best=-1;
  battle.enemies.forEach(e=>{if(e.dead)return;const d=Math.hypot(e.x-t.x,e.y-t.y);if(d<=range){const prog=e.seg+(1-d/1000);if(prog>best){best=prog;target=e}}});
  if(!target)return;
  if(t.key==="hedgehog"){
    battle.needles=battle.needles||[];
    const id=(battle.nextNeedleId=(battle.nextNeedleId||0)+1);
    battle.shots.push({x:t.x,y:t.y-7,tx:target.x,ty:target.y,life:.34,type:"hedgehogNeedles"});
    battle.pendingNeedles=battle.pendingNeedles||[];
    battle.pendingNeedles.push({id,x:target.x,y:target.y,life:.34});
    t.cd=cardRate(t.key);
    return;
  }'''
new='''  let target=null,best=-1;
  battle.enemies.forEach(e=>{if(e.dead)return;const d=Math.hypot(e.x-t.x,e.y-t.y);if(d<=range){const prog=e.seg+(1-d/1000);if(prog>best){best=prog;target=e}}});
  if(t.key==="hedgehog"){
    // Hedgehog lays traps even when no enemy is nearby. Pick a point on the road inside its range.
    let tx,ty;
    if(target){tx=target.x;ty=target.y;}
    else{
      let candidates=[];
      for(let i=0;i<path.length-1;i++){
        const [ax,ay]=path[i],[bx,by]=path[i+1];
        for(let j=0;j<=8;j++){
          const q=j/8,x=ax+(bx-ax)*q,y=ay+(by-ay)*q;
          if(Math.hypot(x-t.x,y-t.y)<=range)candidates.push([x,y]);
        }
      }
      if(!candidates.length){t.cd=.5;return;}
      const pick=candidates[Math.floor(Math.random()*candidates.length)];tx=pick[0];ty=pick[1];
    }
    battle.needles=battle.needles||[];
    const id=(battle.nextNeedleId=(battle.nextNeedleId||0)+1);
    battle.shots.push({x:t.x,y:t.y-7,tx,ty,life:.34,type:"hedgehogNeedles"});
    battle.pendingNeedles=battle.pendingNeedles||[];
    battle.pendingNeedles.push({id,x:tx,y:ty,life:.34});
    t.cd=cardRate(t.key);
    return;
  }
  if(!target)return;'''
if old not in s: raise SystemExit('current hedgehog tower block not found')
s=s.replace(old,new,1)
p.write_text(s)
