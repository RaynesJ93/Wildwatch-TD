...
s=s.replace('hedgehog:{name:"Hedgehog",emoji:"🦔",rarity:"Common",cost:60,range:135,rate:3,dmg:12,color:"#76543b"','hedgehog:{name:"Hedgehog",emoji:"🦔",rarity:"Common",cost:60,range:135,rate:3,dmg:12,color:"#76543b"',1)
old='''  let target=null,best=-1;
  ...
  if(t.key==="hedgehog"){
    ...
    battle.pendingNeedles=battle.pendingNeedles||[];
    battle.pendingNeedles.push({id,x:target.x,y:target.y,life:.34});
    t.cd=cardRate(t.key);
    return;
  }'''
  ...
  if(t.key==="hedgehog"){
    ...
    if(target){tx=target.x;ty=target.y;}
      ...
      for(let i=0;i<path.length-1;i++){
        ...
        for(let j=0;j<=12;j++){
          const q=j/12,x=ax+(bx-ax)*q,y=ay+(by-ay)*q;
          if(Math.hypot(x-t.x,y-t.y)<=range)candidates.push([x,y]);
        }
      }
      if(!candidates.length){t.cd=.5;return;}
      const pick=candidates[Math.floor(Math.random()*candidates.length)];tx=pick[0];ty=pick[1];
    }
    const id=(battle.nextNeedleId=(battle.nextNeedleId||0)+1);
    battle.shots.push({x:t.x,y:t.y-7,tx,ty,life:.34,type:"hedgehogNeedles"});
    battle.pendingNeedles=battle.pendingNeedles||[];
    battle.pendingNeedles.push({id,x:tx,y:ty,life:.34});
    t.cd=cardRate(t.key);
    return;
  }
  if(!target)return;'''
if old not in s:
    raise SystemExit('LIVE hedgehog block not found; refusing unsafe patch')
